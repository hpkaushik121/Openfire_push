...
import org.jivesoftware.Data;
import org.jivesoftware.FCMModel;
import org.jivesoftware.Notification;
...
import org.jivesoftware.util.MessageModel;
    ...
    public void routePacket(JID jid, Packet packet, boolean fromServer) throws PacketException {
        ...
        try {
            ...
            try{
                Message msg=(Message)packet;
                    ...
                    if(msg.getBody()!=null) {
                        MessageModel messageModel = new Gson().fromJson(msg.getBody(), MessageModel.class);
                        messageModel.setTimeStamp(gmtTime);
                        Log.error("packet changed" + new Gson().toJson(messageModel));
                        if (messageModel.getMimeType() != null && (messageModel.getMimeType().equalsIgnoreCase("ACK_DEL")||messageModel.getMimeType().equalsIgnoreCase("ACK_READ"))) {
                            isACK = true;
                        }
                        msg.setBody(new Gson().toJson(messageModel));

                        Log.error("packet changed"+ msg.toXML());
                    }
            ...
            if (Log.isDebugEnabled()) {
                Log.debug("Logging packet: {} packet: {}", jid, packet.toXML());
            }
        ...
        if (!routed) {
            ...
            if (packet instanceof IQ) {
                Log.debug("Failed IQ to JID: {} packet: {}", jid, packet.toXML());
                iqRouter.routingFailed(jid, packet);
            }
            else if (packet instanceof Message) {
                Log.debug("Failed packet to JID: {} packet: {}", jid, packet.toXML());
                messageRouter.routingFailed(jid, packet);
            }
            else if (packet instanceof Presence) {
                Log.debug("Presence not Fund"+packet.toXML());
...
//        }
        ...
        try {
            ...
            ClientSession clientSession=manager.getSession(msg.getTo());
            Log.debug("message type "+msg.getType());
            if((!routed || manager.getDetachedSessions().containsKey(clientSession.getStreamID()))&&msg.getBody()!=null&&!isACK){
                HttpPost newRequest=new HttpPost("https://fcm.googleapis.com/fcm/send");
                String serverKey="{{{ SERVER TOKEN}}}";
                newRequest.addHeader(HttpHeaders.CONTENT_TYPE, "application/json");
                newRequest.addHeader(HttpHeaders.AUTHORIZATION, "Bearer "+serverKey);
                FCMModel model=new FCMModel();
                model.setTo("/topics/"+packet.getTo().toString().split("@")[0]);
                Data data=new Data();
                data.setFrom(packet.getFrom().toString());
                data.setMessage(msg.getBody());
                data.setStanzaID(msg.getID());
                model.setData(data);
                Log.debug("sending notification "+new Gson().toJson(model));


                HttpClient httpClient = HttpClientBuilder.create().build();
                StringEntity params =new StringEntity(new Gson().toJson(model));
                newRequest.setEntity(params);
                HttpResponse response=httpClient.execute(newRequest);
                HttpEntity entity = response.getEntity();
                String responseString = EntityUtils.toString(entity, "UTF-8");
                Log.debug("notification response "+responseString);
    ...
    private boolean routeToBareJID(JID recipientJID, Message packet, boolean isPrivate) {
        ...
        if (nonNegativePrioritySessions.isEmpty()) {
            ...
            Log.debug("Unable to send the packet. No session is available so store offline. {} ", packet.toXML());
            return false;
        }
